...
export default function RolePlayRoulette({ levelFilter, ageGroupFilter, refreshTrigger }: Props) {
  ...
  const fetchData = async () => {
    let query = supabase.from('roleplay_prompts').select('*');
  ...
  const handleDelete = async (id: number) => {
    ...
    const { error } = await supabase.from('roleplay_prompts').delete().eq('id', id);
    if (!error) fetchData();
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full">
      <div className="bg-white border border-slate-200 shadow-sm w-full max-w-[800px] flex flex-col items-center relative overflow-hidden p-10 mx-auto rounded-3xl">
        ...
        <div className="bg-slate-50 w-full p-8 rounded-3xl border border-slate-200 shadow-inner mb-8 text-center">
          <h2 className="text-xl text-slate-800 font-bold uppercase tracking-widest mb-4">Situation</h2>
          <p className="text-3xl font-bold m-0 text-slate-800 leading-relaxed break-words">
            {p.situation}
          </p>
        </div>

        <div className="bg-slate-50 w-full p-8 rounded-3xl border border-slate-200 shadow-inner text-center">
          <h2 className="text-xl text-emerald-600 font-bold uppercase tracking-widest mb-4">Your Role</h2>
          <p className="text-3xl font-bold m-0 text-slate-800 leading-relaxed break-words">
            {p.role}
          </p>
        </div>
